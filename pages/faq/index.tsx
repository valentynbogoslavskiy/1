...
const Faq: FC<FaqPageContent> = (faqContentSSR) => {
  const [pageContent, setPageContent] = useState(faqContentSSR);
  const { header, footer, contentLoaded, setCommonPageContent } = useStore('CommonPageContentStore');
  useEffect(() => {
    if (faqContentSSR && !contentLoaded) setCommonPageContent({ header, footer });
  }, []);
...
export async function getServerSideProps(): Promise<FaqPageProps | null> {
  const response = await fetch(`${process.env.NEXT_PUBLIC_ENV_API_URL}/joinus`);
  const faqContentSSR: FaqPageContent = await response.json();
  return { props: faqContentSSR };
}
