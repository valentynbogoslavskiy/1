...
interface JoinUsPageProps {
  props: JoinUsPageContent | null;
}

interface JoinUsPageContent {
  title: string;
  header: HeaderProps;
  footer: FooterProps;
}
...
const JoinUs: FC<JoinUsPageContent> = (joinUsContentSSR) => {
  const [pageContent, setPageContent] = useState(joinUsContentSSR);
  const { header, footer, contentLoaded, setCommonPageContent } = useStore('CommonPageContentStore');
  useEffect(() => {
    if (joinUsContentSSR && !contentLoaded) setCommonPageContent({ header, footer });
  }, []);
...
export async function getServerSideProps(): Promise<JoinUsPageProps | null> {
  const response = await fetch(`${process.env.NEXT_PUBLIC_ENV_API_URL}/joinus`);
  const joinUsContentSSR: JoinUsPageContent = await response.json();
  return { props: joinUsContentSSR };
}
