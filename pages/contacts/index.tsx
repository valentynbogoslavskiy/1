...
interface ContactsPageProps {
  props: ContactsPageContent | null;
}
...
const Contacts: FC<ContactsPageContent> = (contactsContentSSR) => {
  const [pageContent, setPageContent] = useState(contactsContentSSR);
  const { header, footer, contentLoaded, setCommonPageContent } = useStore('CommonPageContentStore');
  useEffect(() => {
    if (contactsContentSSR && !contentLoaded) setCommonPageContent({ header, footer });
  }, []);
...
export async function getServerSideProps(): Promise<ContactsPageProps | null> {
  const response = await fetch(`${process.env.NEXT_PUBLIC_ENV_API_URL}/joinus`);
  const contactsContentSSR: ContactsPageContent = await response.json();
  return { props: contactsContentSSR };
}
