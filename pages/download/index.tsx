...
interface DownloadPageProps {
  props: DownloadPageContent | null;
}
...
const Download: FC<DownloadPageContent> = (downloadContentSSR) => {
  const [pageContent, setPageContent] = useState(downloadContentSSR);
  const { header, footer, contentLoaded, setCommonPageContent } = useStore('CommonPageContentStore');
  useEffect(() => {
    if (downloadContentSSR && !contentLoaded) setCommonPageContent({ header, footer });
  }, []);
...
export async function getServerSideProps(): Promise<DownloadPageProps | null> {
  const response = await fetch(`${process.env.NEXT_PUBLIC_ENV_API_URL}/joinus`);
  const downloadContentSSR: DownloadPageContent = await response.json();
  return { props: downloadContentSSR };
}
